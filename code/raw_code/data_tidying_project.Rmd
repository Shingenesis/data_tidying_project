...
This project is an exercise of many of the skills needed for succesfully fuifill the data tyding part of a data science project.

### Loading the packages

In the next data chunk the necessary packages are loaded.
...
We are going to work with two datasets. They are associated with retail sales.
...
To be safe and not depend on the availability of the data on the Internet, better save it inside the project.
...
In the next code chunk, the data will be wrangle.
...
df_department <- df1 %>% 
  ...
  mutate(value=as.numeric(value)) %>% summarise(n=sum(value)) %>% 
...
Here, we are going to merge the different datasets into a new one.

```{r merge-data, warning=FALSE} 
## Now, combine the two data frames
df_total <- merge(df_retail,df_department, all=TRUE)
```

#### Ploting the data

As an advance of later course, just run the next code chunk to plot some aspect of the data.
